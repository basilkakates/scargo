...
struct ConnectionResponse {
    ...
    duplicate_count: i64,
    sync_state: String,
    sync_requested_at: Option<String>,
...
struct ConnectionRow {
    ...
    duplicate_count: i64,
    sync_state: String,
    sync_requested_at: Option<DateTime<Utc>>,
        ...
        .execute(
            "INSERT INTO dropbox_connection
                (id, account_id, dropbox_account_id, root_path, encrypted_refresh_token,
                 cursor, status, sync_state, sync_requested_at, sync_started_at,
                 last_sync_at, last_success_at, latest_error, created_at, updated_at)
             VALUES ($1, $2, $3, $4, $5, NULL, 'active', 'idle', NULL, NULL,
...
pub(crate) async fn sync_now(
    ...
    let updated = client
        .execute(
            "UPDATE dropbox_connection
             SET sync_state = CASE WHEN sync_state = 'running' THEN 'running' ELSE 'queued' END,
    ...
    drop(client);
    dropbox_worker::spawn_account_sync(db.get_ref().clone(), cfg.clone(), account.id);
    let client = db.get().await?;
    let row = load_connection(&client, account.id).await?;
    Ok(HttpResponse::Accepted().json(connection_response(Some(cfg), row)))
}

fn authorize_url(cfg: &DropboxConfig, state: &str) -> Result<String, Error> {
    let mut url = Url::parse(DROPBOX_AUTHORIZE_URL).map_err(|_| Error::Internal)?;
    url.query_pairs_mut()
        .append_pair("client_id", &cfg.app_key)
        .append_pair("response_type", "code")
        .append_pair("token_access_type", "offline")
        .append_pair("state", state)
        .append_pair("redirect_uri", &callback_url(cfg));
    Ok(url.to_string())
}
...
async fn exchange_code(cfg: &DropboxConfig, code: &str) -> Result<DropboxTokenResponse, Error> {
    ...
    serde_json::from_slice::<DropboxTokenResponse>(&body).map_err(|_| Error::Database)
}
...
async fn load_connection(
    ...
    let row = client
        .query_opt(
            "SELECT c.status,
             ...
             GROUP BY c.id, c.status, c.root_path, c.last_sync_at, c.last_success_at,
                      c.latest_error, c.sync_state, c.sync_requested_at, c.sync_started_at",
            &[&account_id],
        )
        .await
        .map_err(|_| Error::Database)?;
    ...
    Ok(row.map(|row| ConnectionRow {
        ...
        sync_requested_at: row.get(8),
        sync_started_at: row.get(9),
    }))
}
...
fn connection_response(
    ...
    match row {
        Some(row) => ConnectionResponse {
            ...
            sync_requested_at: row.sync_requested_at.map(|value| value.to_rfc3339()),
            sync_started_at: row.sync_started_at.map(|value| value.to_rfc3339()),
        },
...
pub(crate) fn encrypt_token(key: &[u8; 32], plaintext: &str) -> Result<String, Error> {
...
pub(crate) fn decrypt_token(key: &[u8; 32], ciphertext: &str) -> Result<String, Error> {
...
mod tests {
    ...
    fn redirect_path_must_stay_relative() {
        assert_eq!(
            validate_redirect_path("/dropbox.html?tab=dropbox").unwrap(),
            "/dropbox.html?tab=dropbox"
        );
